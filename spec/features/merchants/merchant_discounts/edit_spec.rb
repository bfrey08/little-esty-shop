...
RSpec.describe 'merchant discounts edit page' do
  ...
  xit 'can display the merchant discount show page' do
    visit("/merchants/#{@merchant_1.id}/discounts/#{@merchant_discount_1.id}/edit")

    fill_in 'Name', with: 'Black Thursday'
    fill_in 'percentage', with: 0.35
    fill_in 'threshold', with: 20
    click_on 'Update Discount'

    expect(current_path).to eq("/merchants/#{@merchant_1.id}/discounts")
    expect(page).to have_content("Black Thursday")
  end
end
